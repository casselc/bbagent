...
plausible filenames, read them successfully, and reported them as the project's
contents. The fixture happens to contain exactly those two files, so the answers
are **accidentally correct**. With a third file present, all three would have
been confidently wrong. The last quote is worse than wrong: it converts "my read
...
3.3, errors from 10.7 to 1.7, and the fastest mean time of any variant.
...
Suite: **107 tests, 803 assertions, 0 failures.**
...
- one model, one endpoint, one prompt, three repetitions per variant. This is a
  ...
  not;
- one fixture project, whose two files the model could plausibly guess. A
  fixture with unguessable names would likely separate the variants further,
  and would have exposed `:generated`'s unsupported claims as outright wrong
...
One caution for A2: when `project/list` exists, the grounding constraint's
wording (*"You cannot enumerate a directory"*) becomes false and must be
regenerated from the catalog rather than left as a constant. That is the same
trap as handwritten capability prose, one level up. The constraint should
eventually derive from which effects the context actually grants.
